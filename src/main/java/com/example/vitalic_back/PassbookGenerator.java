    ...
    public void run(String... args) throws Exception {
        ...
        while (!currentDate.isAfter(endDate)) {
            ...
            for (int i = 0; i < 2 + new Random().nextInt(2); i++) {
                // Random deposits
                if (Math.random() < 0.11) {
                    int tranAmt = randomAmount(10000, 12000);
                ...
                if (Math.random() < 0.12) {
                    int tranAmt = randomAmount(10000, 12000);
                    String convenient = getRandomConvenientStore();
                    LocalDateTime randomTranDateTime = generateRandomTime(currentDate, false);
                    sqlQueries.append(createTransaction("신한은행", "110490816690",
                            currentBalance, 1, convenient, "내 신한은행 110490816690",2, randomTranDateTime, 1, tranAmt));
                }
                if (Math.random() < 0.11) {
                    int tranAmt = randomAmount(10000, 12000);
                    String webShop = getRandomWebShop();
                    LocalDateTime randomTranDateTime = generateRandomTime(currentDate, false);
                    sqlQueries.append(createTransaction("신한은행", "110490816690",
                            currentBalance, 1, webShop, "내 신한은행 110490816690",4, randomTranDateTime, 1, tranAmt));
                }
                if (Math.random() < 0.11) {
                    int tranAmt = randomAmount(10000, 12000);
                    String entertain = getRandomEntertainment();
                    LocalDateTime randomTranDateTime = generateRandomTime(currentDate, false);
                    sqlQueries.append(createTransaction("신한은행", "110490816690", currentBalance,
                            1, entertain, "내 신한은행 110490816690",5, randomTranDateTime, 1, tranAmt));
                }
            }
